...
/*   Updated: 2025/12/29 17:48:03 by mben-cha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void camera_move_forward_backward(t_game_state *game, double moveSpeed)
{
    if (game->keys['W'] == 1)
    {
        if (game->config->map.grid[(int)(game->posX + game->dirX * moveSpeed)][(int)game->posY] == 0)
            game->posX = game->posX + game->dirX * moveSpeed;
        if (game->config->map.grid[(int)game->posX][(int)(game->posY + game->dirY * moveSpeed)] == 0)
...
void camera_move_left_right(t_game_state *game, double moveSpeed)
{
    if (game->keys['A'] == 1)
    {
        if (game->config->map.grid[(int)(game->posX - game->dirY * moveSpeed)][(int)game->posY] == 0)
    ...
    if (game->keys['D'] == 1)
    {
        ...
        if (game->config->map.grid[(int)game->posX][(int)(game->posY - game->dirX * moveSpeed)] == 0)
            game->posY = game->posY - game->dirX * moveSpeed;
    }
}
